...
function extract_features(processed_data, T)
    ...
    features_edge_positive_negative = Matrix{Float64}(undef, num_samples, 8100)
    ...
    for i in 1:num_samples
        ...
        sum_output_edge_negative_negative = zeros(90, 90)
        ...
        for t in 1:T-1
            # 获取当前时间点的数据
            list_all_simplices_positive, list_all_simplices_negative,edges_matrix_positive,edges_matrix_negative,edges_matrix_negative_negative = create_simplicial_complex(simplicial_TS, t)
        
            #sum_output_edge_positive .+= edges_matrix_positive
            sum_output_edge_negative_negative .+= edges_matrix_negative_negative
        ...
        avg_output_edge_negative_negative = sum_output_edge_negative_negative / (T - 1)
       
        features_edge_positive_negative[i, :] = avg_output_edge_negative_negative[:]

       
    end
    return features_edge_positive_negative
end
...
#println(join(output, ' '))
#println(size(output))
AD_features_edge_positive_negative = extract_features(processed_data_AD, T)
println("=======================AD is ok")
CN_features_edge_positive_negative = extract_features(processed_data_CN, T)
println("=======================")
MCI_features_edge_positive_negative = extract_features(processed_data_MCI, T)
println("=======================")
...
# 保存 features_AD
output_file_ad1 = "AD_features_edge_positive_negative.txt"
writedlm(output_file_ad1, AD_features_edge_positive_negative, '\t')  # 使用制表符分隔



# 保存 features_CN
output_file_CN1 = "CN_features_edge_positive_negative.txt"
writedlm(output_file_CN1, CN_features_edge_positive_negative, '\t')  # 使用制表符分隔


# 保存 features_MCI
output_file_MCI1 = "MCI_features_edge_positive_negative.txt"
writedlm(output_file_MCI1, MCI_features_edge_positive_negative, '\t')  # 使用制表符分隔
