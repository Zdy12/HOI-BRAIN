module SimplicialTS_NEW_interper
...
#升序排列，观察向下闭包
    ...
    if counter_simplices_all > 0 
       num_2d_holes =compute_scaffold(simplicial_TS,list_simplices_for_filtration,list_simplices_all,t_current)
    end
    return num_2d_holes
end
...
function compute_scaffold(simplicial_TS::simplicial_complex_mvts,  list_simplices_positive::Vector{Pair{Tuple{Int64, Vararg{Int64}}, Float64}},
     list_filtration_scaffold::Vector{Pair{Tuple{Int64, Vararg{Int64}}, Float64}},
      ...
      =#
        ...
        finite_h2 = filter(d -> isfinite(d.birth) && isfinite(d.death), h2)


        num_2d_holes = count(d -> d.death > d.birth, finite_h2)

        println("二维洞的个数为：", num_2d_holes)      
...
function search_good_quadruplet(sorted_simplices::Vector{Tuple{Vector{Int64}, Float64}})
    # 按权重升序排序
